...
// This program implements a simple curency lookup service
...
// Clients send currency search requests as a textual command in
// the form:

// GET <currency, country, or code>

// When the server receives the request, it is parsed and is then
// used to search the list of currencies. The search result is then
// printed line-by-line back to the client.

// Focus:
// This version of the server uses TCP sockets (or UDS) to implement 
// a simple text-based application-level protocol. There are no streaming
// strategy employed for the read/write operations. Buffers are read in 
// one shot creating opportunities for missing data during read.
...
func main() {
   var addr string
   var network string
   flag.StringVar(&addr, "e", ":4040", "service endpoint [ip addr or path]")
...
func handleConnection(conn net.Conn) {
   ...
   for {
      ...
      case "GET":
         ...
         // send each currency info as a line to the cient with fmt.Fprintf()
